...
CREATE USER jjm_sample IDENTIFIED BY sample1234;

-- 2. 새 데이터베이스 연결(접속) 추가
-- 사용자 jjm_sample는 CREATE SESSION 권한을 가지고 있지 않음;
-- > 접속 권한 부여해야함

-- 3. 접속 권한 부여(sys -> 사용자계정)
-- [권한 부여 작성법]
-- GRANT 권한, 권한,... TO 사용자명;
GRANT CREATE SESSION TO jjm_sample;
...
GRANT CREATE TABLE TO jjm_sample;

ALTER USER jjm_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
-- SYSTEM 테이블스페이스를 사용자 jjm_sample의 저장소로 무제한 사용 가능하게끔 하는 명령어
...
GRANT CONNECT, RESOURCE TO jjm_sample;
...
SELECT * FROM kh_jjm.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
-- > sample 계정은 kh 계정의 테이블 접근 권한이 없어서 조회 불가

-- 2. (kh) sample 계정에 EMPLOYEE 테이블 조회 권한 부여
-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO jjm_sample;

-- 3. (sample) 다시 kh계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh_jjm.EMPLOYEE;

-- 4. (kh) sample 계정에 부여했던 EMPLOYEE 테이블 조회권한 회수(REVOKE)
REVOKE SELECT ON EMPLOYEE FROM jjm_sample;
