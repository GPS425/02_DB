...
-- 실제로 아직 DB에 반영된 것이 아니다.
-- 트랜잭션에 INSERT 3개 들어있음

ROLLBACK;	-- 위에 추가한 3개 사라짐
...
ROLLBACK;	-- 직전에 COMMIT한 직후의 순간으로 돌아가기 때문에 데이터가 사라지지 않음
...
SAVEPOINT "SP2";	-- 세이브포인트 지정

INSERT INTO DEPARTMENT2 VALUES('T6', '개발6팀', 'L2');
SAVEPOINT "SP3";	-- 세이브포인트 지정
...
-- ROLLBACK TO "SP2" 	-- >> SP2, SP3 모두 함께 삭제됨
...
SELECT * FROM DEPARTMENT2;	-- 개발5팀까지 남아있음
...
SELECT * FROM DEPARTMENT2;	-- 개발1,2,3팀만 남아있음
