...
/* 3 */SELECT JOB_CODE, ROUND(AVG(SALARY)), COUNT(*)
/* 1 */FROM EMPLOYEE
/* 2 */GROUP BY JOB_CODE
/* 4 */ORDER BY JOB_CODE ASC;


-- EMPLOYEE 테이블에서 (FROM)
-- 성별(남/여)과 각 성별 별 인원 수, 급여 합을(SELECT)
-- 인원 수 오름차순으로 조회(ORDER BY)

SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') AS "성별", 
COUNT(*) AS "인원 수", SUM(SALARY) AS "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여')	-- 별칭 사용 X(SELECT 해석 X)
ORDER BY "인원 수";	-- 별칭 사용 가능(SELECT 절 해석 완료된 후이기 때문)
...
-- 부서코드, 평균급여, 인원 수 조회 (SELECT)

SELECT DEPT_CODE AS "부서코드", ROUND(AVG(SALARY)) AS "평균급여", COUNT(*) AS "인원 수"
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5', 'D6')
GROUP BY DEPT_CODE;
-- 부서의(WHERE 절에서 구한 부서코드가 D5, D6인 데이터들을 그룹으로 묶음)
...
-- 그룹함수가 적용되지 않은 컬럼은 모두 GROUP BY절에 작성해야함.
...
-- EMPLOYEE 테이블에서 부서별 평균 급여가 3백만원 이상인 부서의
-- 부서코드, 평균 급여 조회
-- 부서코드 오름차순

SELECT DEPT_CODE, ROUND(AVG(SALARY)) "평균 급여"
FROM EMPLOYEE
-- WHERE SALARY >= 300000 >> 한 사람의 급여가 3백만원 이상이라는 조건 (요구사항 X)
GROUP BY DEPT_CODE
