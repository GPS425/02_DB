 ...
 * 
 * -- 테이블에서 수정할 수 있는 것
 * 1) 제약조건(추가/삭제) * 제약 조건 자체를 수정하는 구문은 별도 존재하지 않음
 * 					  * 삭제 후 추가해야함..
 * 2) 컬럼(추가/수정/삭제)
...
-- DEPARTMENT 테이블 복사(컬럼명, 데이터타입, NOT NULL만 복사됨)
...
-- DEPT_COPY의 DEPT_TITLE 컬럼에 UNIQUE 제약 삭제
ALTER TABLE DEPT_COPY
...
-- ORA-12899: "KH_JJM"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
-- > DEPT_ID의 데이터타입이 CHAR(2)이므로 영어 + 숫자 2글자까지 저장 가능
-- > D10 은 영어 + 숫자 3글자
-- >> VARCHAR2(3)으로 변경해보기 (남는 바이트 메모리 반환)

-- DEPT_ID 컬럼 데이터 타입 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR(3);

SELECT * FROM DEPT_COPY
-- >> 컬럼의 데이터 타입 변경 후 위 INSERT 수행 >> 삽입 성공
...
-- DEPT_COPY 테이블에 PK 추가 (컬럼 : DEPT_ID, D_COPY_PK)
ALTER TABLE DEPT_COPY
...
-- TB2에 샘플 데이터 삽입
INSERT INTO TB2 VALUES(11, 1);		-- 1 : FK
...
-- > 해결 방법
-- 1) 자식, 부모 순으로 삭제
-- 2) ALTER를 이용해서 FK 제약조건 삭제 후 부모테이블 삭제
...
-- TB2 테이블은 독립적인 테이블로 혼자 남게됨(아무 관계없음)
...
-- 롤백 안됨.
-- 위에서 DDL 구문 중 ALTER를 사용하여 그 시점에 COMMIT 됨.
