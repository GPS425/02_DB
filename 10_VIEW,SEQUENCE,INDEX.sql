...
GRANT CREATE VIEW TO kh_jjm;


-- VIEW 생성	(kh_jjm 계정으로 접속 후 진행)
CREATE VIEW V_EMP
...
-- > 모든 컬럼 값이 INSERT된 것이 아니라
-- VIDW를 생성할 때 사용된 컬럼에만 데이터가 삽입되었고
-- 반대로, 사용되지 않은 컬럼(DEPT_TITLE)에는 NULL이 들어감
-- > NULL은 DB의 무결성을 약하게 하는 주요 원인
...
-- > CURRVAL의 정확한 의미는 가장 퇴근 호출된 NEXTVAL 값을 반환함을 뜻함
...
INSERT INTO TB_TEST
VALUES(SEQ_TEST_NO.NEXTVAL, '짱구');	-- 125 짱구
INSERT INTO TB_TEST
VALUES(SEQ_TEST_NO.NEXTVAL, '철수');	-- 130 철수
INSERT INTO TB_TEST
VALUES(SEQ_TEST_NO.NEXTVAL, '유리');	-- 135 유리
 ...
 * 즉, 인덱스의 핵심 역할은
 * 컬럼값 >> 해당 행의 ROWID를 통해 빠르게 찾아내는 것!
 */
...
-- TB_IDX_TEST에 샘플 데이터 100만개 삽입(PL/SQL 사용)
-- 조건문(IF/CASE), 반복문(LOOP, WHILE), 변수 선언 등..
