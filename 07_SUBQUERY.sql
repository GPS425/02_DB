/*
 * SUBQUERY(서브쿼리 == 내부 쿼리)
 * - 하나의 SQL 문에 포함된 또 다른 SQL문
 * - 메인 쿼리(== 외부 쿼리, 기존쿼리)를 위해 보조 역할을 하는 쿼리문
 * 
 * -메인쿼리가 SELECT문일때
...
-- 2) 직원 중 급여가 3047663 이상인 사원들의 사번, 이름, 직급코드, 급여 조회
...
-- FROM, JOIN으로 인해 테이블 상 존재하는 컬럼이면
-- ORDER BY 절 사용 가능!!
...
-- 1) 사수에 해당하는 사원 번호 조회(SUBQUERY)
SELECT DISTINCT(MANAGER_ID)
FROM EMPLOYEE
WHERE MANAGER_ID IS NOT NULL;

-- 2) 직원의 사번, 이름, 부서명, 직급명 조회(MAIN QUERY)
...
-- 2) 선택 함수 사용
-- > DECODE(컬럼명, 값1, 1인 경우, 값2, 2인경우, ... , 일치하지 않는 경우)
...
-- 3) EMPLOYEE 테이블의 위의 결과와 동일한 DEPT_CODE를 가진 사원의 이름, 부서코드 조회
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
-- 3. (단일행) 다중열 서브쿼리
-- 서브쿼리 SELECT 절에 나열된 컬럼수가 여러 개 일때
...
WHERE (DEPT_CODE, JOB_CODE) IN (
						        ...
						        FROM
						            EMPLOYEE
						        WHERE
						            SUBSTR(TO_CHAR(HIRE_DATE, 'YYYYMMDD'), 1, 4) = '2000'
						  		);
...
-- 본인이 조속된 직급의 평균 급여를 받고 있는 직원의
-- 사번, 이름, 직급코드, 급여 조회
-- 단, 급여와 급여 평균은 만원 단위로 계산 TRUNC()
...
-- 부서 별 입사일이 가장 빠른 사원의
-- 사번, 이름, 부서코드, 부서명(NULL이면 '소속없음'), 직급명, 입사일 조회
-- 입사일 빠른순으로 정렬
-- 단, 퇴사한 직원은 제외할 것


-- 메인 쿼리
SELECT EMP_ID, EMP_NAME, DEPT_CODE, NVL(DEPT_TITLE, '소속없음') 부서명, JOB_NAME, HIRE_DATE
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
ORDER BY HIRE_DATE ASC;

-- SUBQUERY (부서별 입사일이 가장 빠른 사원)
...
-- 모든 직원의 이름, 직급, 급여, 전체 사원 중 가장 높은 급여와의 차 조회
SELECT EMP_NAME, JOB_CODE, SALARY,
...
FROM (SELECT EMP_NAME, SALARY
		FROM EMPLOYEE
		ORDER BY SALARY DESC)	-- 해석순위 1위 FROM절에서 전체 직원의 SALARY 내림차순 정렬
...
-- EX) 공동 1위가 2명이면 다음 순위는 2위가 아닌 3위로 조회됨
...
SELECT MAIN.EMP_ID AS 사번, MAIN.EMP_NAME AS 이름, NVL(D.DEPT_TITLE, '소속없음') AS 부서명,
J.JOB_NAME AS 직급명, TO_CHAR(MAIN.HIRE_DATE, 'YYYY-MM-DD') AS 입사일
FROM EMPLOYEE MAIN
JOIN JOB J USING(JOB_CODE)
LEFT JOIN DEPARTMENT D ON(MAIN.DEPT_CODE = D.DEPT_ID)
WHERE MAIN.ENT_YN = 'N'
AND MAIN.HIRE_DATE = (SELECT MIN(HIRE_DATE)
				      FROM EMPLOYEE SUB
				      WHERE NVL(MAIN.DEPT_CODE, 'N/A') = NVL(SUB.DEPT_CODE, 'N/A')
				      AND SUB.ENT_YN = 'N')
ORDER BY 입사일 ASC;
...
(EXTRACT(YEAR FROM SYSDATE) - (CASE WHEN SUBSTR(MAIN.EMP_NO, 8, 1)
							   IN ('1', '2') THEN 1900
							   ELSE 2000
							   END + SUBSTR(MAIN.EMP_NO, 1, 2))) AS 나이,
TO_CHAR((MAIN.SALARY + NVL(MAIN.SALARY * MAIN.BONUS, 0)) * 12,
				 'L999,999,999', 'NLS_CURRENCY=''￦'''
		) AS "보너스 포함 연봉"
FROM EMPLOYEE MAIN
JOIN JOB J ON (MAIN.JOB_CODE = J.JOB_CODE)
WHERE EMP_NO = (SELECT MAX(SUB.EMP_NO)
				FROM EMPLOYEE SUB
