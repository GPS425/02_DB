...
-- INSERT 시 가입일 미작성하는 경우 DEFAULT 값이 반영되는 지 확인
INSERT INTO MEMBER (MEMBER_ID, MEMBER_PWD, MEMBER_NAME)
VALUES('MEM03', 'ASDF1234', '이지연');
...
--   0으로 시작할 가능성이 있다면 CHAR, VARCHAR와 같은 문자열 이용
...
-- 1. NOT NULL
-- 해당 컬럼에 반드시 값이 기록되어야 하는 경우 사용함
-- 삽입/수정 시 NULL 값을 허용하지 않도록 컬럼레벨에서 제한

-- * 컬럼레벨 : 테이블 생성 시 컬럼을 정의하는 부분에 작성하는 것

CREATE TABLE USER_USED_NN(
	USER_NO NUMBER NOT NULL,	-- 모든 사용자는 사용자 번호가 있어야 한다.
	USER_ID VARCHAR2(20),
...
-- NULL을 ("KH_JJM"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다

--------------------------------------------------------------------------------

-- 2. UNIQUE 제약조건
-- 컬럼의 입력값에 대해서 중복을 제한하는 제약 조건
...
INSERT INTO USER_USED_UK VALUES
(1, 'USER01', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- 무결성 제약 조건(KH_JJM.USER_ID_U)에 위배됩니다


INSERT INTO USER_USED_UK VALUES
(1, NULL, 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- >> 아이디에 NULL값 삽입 가능 확인(UNIQUE 제약 조건 있어도 NULL값 허용 가능)
...
INSERT INTO USER_USED_UK2 VALUES
(1, 'USER01', 'PASS01', '고길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
...
INSERT INTO USER_USED_UK2 VALUES
(1, 'USER02', 'PASS01', '고길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- 무결성 제약 조건(KH_JJM.USER_ID_NAME_U)에 위배됩니다

-- >> 복합키로 지정된 컬럼값 중 하나라도 다르면 위배되지 않음
--    하지만 모든 컬럼의 값이 중복되면 위배된다.
...
-- 테이블에 대한 고유 식별자(회원 번호, 학번..) 역할을 함

-- NOT NULL + UNIQUE 제약조건의 의미 >> 중복되지 않는 값이 필수로 존재해야함

-- 한 테이블 당 한 개 설정 가능
-- 컬럼레벨, 테이블레벨 둘 다 설정 가능
-- 한 개 컬럼에 설정할 수 있고, 복합키로도 설정 가능함(PRIMARY 복합키)
...
INSERT INTO USER_USED_PK VALUES
(1, 'USER02', 'PASS02', '이순신', '남자', '010-1234-2678', 'lee@kr.or.kr');
-- 무결성 제약 조건(KH_JJM.USER_NO_PK)에 위배됩니다
-- 기본 키 중복 오류

INSERT INTO USER_USED_PK VALUES
(NULL, 'USER02', 'PASS02', '이순신', '남자', '010-1234-2678', 'lee@kr.or.kr');
-- ORA-01400: NULL을 ("KH_JJM"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
-- 기본 키는 NULL을 가질 수 없다.


INSERT INTO USER_USED_PK VALUES
(2, 'USER02', 'PASS02', '이순신', '남자', '010-1234-2678', 'lee@kr.or.kr');
...
INSERT INTO USER_USED_PK2 VALUES
(2, 'USER02', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_JJM.PK_USERNO_USERID)에 위배됩니다
-- >> USER_NO, USER_ID 둘 다 중복일 때 제약조건 위배


INSERT INTO USER_USED_PK2 VALUES
(NULL, 'USER02', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-01400: NULL을 ("KH_JJM"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다

INSERT INTO USER_USED_PK2 VALUES
(3, NULL, 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-01400: NULL을 ("KH_JJM"."USER_USED_PK2"."USER_ID") 안에 삽입할 수 없습니다

-- >> 복합키 2개 중 하나라도 NULL이라면 제약조건 위배


--------------------------------------------------------------------------------

-- 4. FOREIGN KEY(외래 키 / 외부 키)
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
-- FOREIGN KEY 제약조건에 의해서 테이블간의 관계가 형성됨
-- 제공되는 값 외에는 NULL을 사용할 수 있음.

-- 컬럼레벨일 경우
-- 컬럼명 자료형(크기) [CONSTRAINT 이름] REFERENCES 참조할테이블명 [(참조할 컬럼명)] [삭제룰]

-- 테이블 레벨일 경우
-- [CONSTRAINT 이름] FOREIGN KEY (적용할컬럼) REFERENCES 참조할테이블명 [(참조할 컬럼명)] [삭제룰]
...
INSERT INTO USER_USED_FK VALUES
(2, 'USER02', 'PASS02', '이순신', '남자', '010-1232-6678', 'LEE@kr.or.kr', 10);

INSERT INTO USER_USED_FK VALUES
(3, 'USER03', 'PASS03', '유관순', '여자', '010-1542-6678', 'yoo@kr.or.kr', 30);

INSERT INTO USER_USED_FK VALUES
(4, 'USER04', 'PASS04', '안중근', '남자', '010-3232-6123', 'AHN@kr.or.kr', NULL);

INSERT INTO USER_USED_FK VALUES
(5, 'USER05', 'PASS05', '윤봉길', '남자', '010-1222-6327', 'yoon@kr.or.kr', 50);
-- ORA-02291: 무결성 제약조건(KH_JJM.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
-- > 외래키 제약조건에 위배되어 오류 발생(부모의 GRADE_CODE에는 50 없음)
...
-- 1) ON DELECT RESTRICTED(삭제 제한)으로 기본 지정되어있음
-- FOREIGN KEY로 지정된 컬럼에서 사용되고 있는 값일 경우
-- 제공하는 컬럼의 값은 삭제하지 못함

DELETE FROM USER_GRADE WHERE GRADE_CODE = 30;
-- ORA-02292: 무결성 제약조건(KH_JJM.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- 부모테이블인 USER_GRAE2에서 GRADE_CODE 10 삭제

DELETE FROM USER_GRADE2
WHERE GRADE_CODE = 10;

SELECT * FROM USER_GRADE2;		-- 10 삭제 됨
SELECT * FROM USER_USED_FK2;	-- 10이었던 자식 레코드 값 NULL로 바뀜
...
CREATE TABLE USER_USED_FK3(
	...
	REFERENCES USER_GRADE3 ON DELETE CASCADE -- 삭제 옵션
);
...
-- ON DELETE CASCADE 삭제 옵션이 설정되어 있어서 오류 없이 삭제됨
...
-- '남자' 를 '남'으로 바꿔야 CHECK 제약 조건에 성립됨

INSERT INTO USER_USED_CHECK VALUES
(2, 'USER02', 'PASS02', '유관순', '여', '010-1542-6678', 'yoo@kr.or.kr');
-- '여자'를 '여'로 바꿔서 수행
