...
-- 윈도우 > 설정 > 연결 > 연결 유형 AUTO COMMIT BY DEFAULT 체크 해제 필수

-- 주의 : 수업 듣다가 혼자 앞서나가서 COMMIT, ROLLBACK 하지 마라.
...
-- 선택 안 된 컬럼은 값이 NULL로 할당됨 (DEFAULT 존재 시 DEFAULT 설정 값으로 삽입됨)
...
ROLLBACK;	-- 트랜잭션에서 이미 COMMIT 된 상태이기 때문에 ROLLBACK이 안됨

SELECT * FROM EMPLOYEE2
WHERE EMP_ID = '900';

-- 조회 다시 해봄(영구 저장 되었기 때문에 롤백한다 해도 되돌리기 안됨)
...
-- 서브쿼리(SELECT) 결과(RESULT SET)를 EMP_01에 INSERT
...
-- EMPLOYEE2 테이블에서 보너스를 받지 않는 사원의 BONUS를 0.1로 수정
...
INSERT INTO EMP_M02
VALUES(999, '곽두원', '561016-1234567', 'kwack_dw@or.kr', 01011112222, 'D9', 'J4', 'S1', 9000000, 0.5, NULL, SYSDATE, NULL, NULL);
...
-- 얘도 WHERE 절 중요함
-- 만약에 WHERE 절 조건을 설정하지 않으면 모든 행이 다 삭제됨!
