...
-- EMPLOYEE 테이블에서 사원의 월급을 100만으로 나눴을때 나머지값 조회
...
-- SYSDATE : 시스템상의 현재 시간(년, 월, 일, 시, 분, 초)를 반환함
SELECT SYSDATE FROM DUAL; -- 2025-10-20 09:15:03.000

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가(UTC 정보)
SELECT SYSTIMESTAMP FROM DUAL;	-- 2025-10-20 09:16:14.242 +0900

-- MONTHS_BETWEEN(날짜, 날짜): 두 날짜의 개월 수 차지 반환
SELECT ABS(ROUND(MONTHS_BETWEEN(SYSDATE, '2026-02-27'), 1)) "수강기간(개월)"
FROM DUAL;	-- 4.2
...
-- ADD_MONTHS(날짜, 숫자) : 날짜에 숫자만큼의 개월 수를 더한다. (숫자값은 음수도 가능하다)

SELECT ADD_MONTHS(SYSDATE, 4) FROM DUAL;	-- 2026-02-20 09:30:17.000
SELECT ADD_MONTHS(SYSDATE, -1) FROM DUAL;	-- 2025-09-20 09:31:32.000


-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함
SELECT LAST_DAY(SYSDATE) FROM DUAL;			-- 2025-10-31 09:32:13.000
...
-- 문자열로 변환 : TO_CHAR(날짜 OR 숫자, [포맷])
-- : 날짜 >> 문자열, 숫자 >> 문자열 둘 다 가능

-- 숫자 >> 문자 변환 시 포맷 패턴
-- 9 : 숫자 한 칸을 의미, 여러 개 작성 시 오른쪽 정렬
-- 0 : 숫자 한 칸을 의미, 여러 개 작성 시 오른쪽 정렬 + 빈 칸 0 추가
...
SELECT TO_CHAR(1000000, 'L9,999,999') "돈" FROM DUAL;	-- ￦1,000,000
...
-- 날짜 표현 시 년, 월, 일과 같은 특수문자가 아닐 시 쌍따옴표를 이용하여 단순한 문자로 인식시키면 해결됨
...
-- TO_DATE(문자, [포맷]) : 문자형 >> 날짜형으로 변경
-- TO_DATE(숫자, [포맷]) : 숫자형 >> 날짜형으로 변경
-- >> 지정된 포맷으로 날짜를 인식함
...
-- >> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜인지 인식시킴

-- Y 패턴 : 현재 세기(21세기 -- 20XX년도 -- 2000년대)
-- R 패턴 : 1세기 기준으로 절반(50년) 이상인 경우에는 이전 세기(1900년대)
...
-- 2) 추출한 생년월일을 TO_DATE 타입으로 변경 >> RR 패턴 이용하여 1900년대로 수정
SELECT EMP_NAME,
...
-- 숫자 형변환
-- TO_NUMBER(문자데이터 [포맷]) : 문자열 데이터를 숫자 데이터로 변경
-- >> 날짜 데이터를 숫자형으로 변경 원할 시
-- TO_CHAR 이용하여 문자형으로 변경 후 TO_NUMBER 숫자로 변경
...
-- NVL(컬럼명, 컬럼값이 NULL일 때 바꿀 값) : NULL인 컬럼 값을 다른값으로 변경

SELECT EMP_NAME, SALARY, NVL(BONUS, 0), SALARY * NVL(BONUS, 0) "보너스"
FROM EMPLOYEE;

-- NULL과 산술 연산을 진행하면 결과는 무조건 NULL

-- NVL2(컬럼명, 바꿀값1, 바꿀값2)
-- >> 해당 컬럼의 값이 있으면 바꿀값1으로, NULL이면 바꿀값2로 변경됨

-- EMPLOYEE 테이블에서 보너스를 받으면 'O' 표시, 안 받으면 'X' 조회
SELECT EMP_NAME, SALARY, NVL(BONUS, 0), SALARY * NVL(BONUS, 0) "보너스", NVL2(BONUS, 'O', 'X')
FROM EMPLOYEE;
...
-- 직원의 급여를 인상하려고 한다
-- 직급 코드가 J7인 직원은 20% 인상, J6인 직원은 15% 인상, J5인 직원은 10% 인상, 그 외 직급은 5% 인상
...
-- 그룹 함수
-- 하나 이상의 행을 그룹으로 묶어 연산하여 종합, 평균 등의
-- 1개의 결과 행으로 반환하는 함수
...
-- 급여 최소값, 가장빠른 입사일, 알파벳순서 상 가장 앞에 있는 이메일 조회
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL) FROM EMPLOYEE;

-- 급여 최대값, 가장늦은 입사일, 알파벳순서 상 가장 뒤에 있는 이메일 조회
